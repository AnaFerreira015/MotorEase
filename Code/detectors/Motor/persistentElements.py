        ...
        if len(dictionary[i]) > 1:
            ...
            for j in dictionary[i]:
                ...
                if pixels == initPixels:
                    if dictionary[i][0][0][0][0] == j[0][0][0] and subtractBounds(dictionary[i][0][0], j[0])[0] != 0 and \
                            subtractBounds(dictionary[i][0][0], j[0])[1] != 0:
                        ...
                        violationDetails.append({
                            "screen": j[1].replace(".xml", ".png"),
                            "element_id": i,
                            "guideline": "Persistent Elements",
                            "bounds": normalize_bounds(j[0])
                        })
                initPixels1.close()
                im.close()
    return [allViolations, noViolations, violationDetails]
