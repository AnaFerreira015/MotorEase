    ...
    if ".DS_S" not in xml_path:
            ...
            if len(elements) > 1:
                if elements[8][0] == 'clickable' and elements[8][1] == 'true' and elements[16][1] != '[0,0][0,0]':
                    ...
                    try:
                        ...
                        right = int(
                            box_string[box_string.index("[", 1) + 1:box_string.index(",", box_string.index(",") + 1)])
                        ...
                        savePath = "./Code/detectors/Visual/UIED-master/data/input/" + str(
                            screenshot_path.split('/')[-1])
                        im1.save(savePath)
                        im1.close()
                        ...
                        if len(extractedText) > 10 and is_single_color_image(savePath) == False:
                            ...
                            for root, dirs, files_in_dir in os.walk(
                                    "./Code/detectors/Visual/UIED-master/data/output/ip/"):
                                for file_name in files_in_dir:
                                    if ".json" in file_name:
                                        data = []
                                        # print(file_name)
                                        with open("./Code/detectors/Visual/UIED-master/data/output/ip/" + file_name,
                                                  "r") as file:
                                            data = json.load(file)
                                        for i in range(len(data["compos"])):
                                            ...
                                            newBox = [bbox[0] + subWidth, bbox[3] + subHeight, bbox[2] - subWidth,
                                                      bbox[1] - subHeight]
        ...
        for i in range(0, len(bounding_boxes)):
            for j in range(i + 1, len(bounding_boxes)):
                if bounding_boxes[i] != bounding_boxes[j] and check_no_matching_numbers(bounding_boxes[i],
                                                                                        bounding_boxes[j]) and (
                        is_overlapping(bounding_boxes[i], bounding_boxes[j]) == False):
        ...
        if len(uniqueDistances.keys()) > 0 and len(uniqueDistances.keys()) < 50:
            for distT, pairs in uniqueDistances.items():
                for pair in pairs:
                    boxA, boxB = pair[0], pair[1]
                    violationBounds.append({
                        "guideline": "Visual Icon Distance",
                        "bounds_pair": [boxA, boxB],
                        "distance": list(distT)
                    })
            print(screenshot_path)
            return [1, violationBounds]
        else:
            return [0, []]
